...
class UsersInfoBloc {
  ...
  final _userFetcher = PublishSubject<
      User>(); // Stream controller that has both a source and a sink

  Stream<User> get allUsers => _userFetcher.stream;

  fetchUserInfo() async {
    User user = await _repository
        .fetchUserInfo(); //TODO: _repository is not an abstract class who why are we able to call a method in it's class without instantiating it
    _userFetcher.sink
